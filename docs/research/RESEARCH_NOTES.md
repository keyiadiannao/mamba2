...
> 来源：与协作者的讨论整理；**作为工作假说与实验设计备忘**，非已证结论。实现时需与 `docs/overview/planning/PROJECT_MASTER_PLAN.md` §1 中「状态快照 / 廉价回溯」对照阅读。
...
**中文摘要（P0，叙事边界）**：阶段 1 **主文素材**来自 **path-batch** 三 reader 基准（含 naive/fused 峰值图与 Wikitext 浅树），度量的是**沿路径批量前向**的耗时与显存峰值，**不是**树上 DFS 试错、**也不是**全 LM KV 分项。**§7.2–§7.3** 与 **§7.3.1** 表为**独立玩具协议**（S1/S2/S3/S4 等），各列计时对象不同，**禁止**与主图曲线混读为同一物理「一步」。**SSGS×Mamba**（§6，`dfs_ssgs_mamba`）是第三条线：token 步进 + cache 快照下的 **DFS 导航环**，登记见 **X-20260421-ssgs-mamba-dfs-demo**。**真 LM 玩具线**（**X-20260422–24**）与 **SSGS** 任务对象不同；**X-20260424** 默认 **reach_rate&lt;1**，勿写成已解决导航；并列对照见 **X-20260425**。详细图注与表注句稿见 **`docs/experiments/phases/FIGURE_CAPTIONS_STAGE1.md`** 篇首「P0 叙事边界」。
...
- **树上导航（目标叶条件、可学习子指针）**：`src/rag_tree/tree_lm_nav_learned.py` — 冻结 **tiny-gpt2**，**最后一 token 隐状态** 与 **`goal_leaf_index` 嵌入** 拼接后线性层 → 各子 **logits**；监督 **CrossEntropy**（默认 8 叶深度 3 二元树 **24** 条内部节点样本）；训练为 **每 epoch 打乱后全量累积梯度、一步 AdamW**。推理时对指定目标叶贪心下降，指标与上行相同。入口：`scripts/research/demo_tree_lm_nav_learned.py`。登记 **X-20260424-tree-lm-nav-learned**；归档 **`tree_lm_nav_learned_default8_cpu.json`** 与 **`tree_lm_nav_learned_default8_cuda.json`**（**CPU/CUDA** 上 **reach_rate** / **mean_child_choice_accuracy** 与 **per_leaf** 模式一致，与 **X-20260423** 同）。与 **X-20260423** 的边界：**必须传入目标叶**（同一前缀在不同目标下金孩子不同）；**非**盲导航。**叙事**：默认超参下 **reach_rate≈0.375**，**勿过度宣称**「已学会树上导航」；见 **`FIGURE_CAPTIONS_STAGE1.md`** P0 段末。
- **SSGS×LM 轨迹对照（玩具）**：`scripts/research/demo_ssgs_lm_nav_compare.py` — 与 **X-20260424** 同款 **8 句叶**、`build_bottom_up_text_tree`（**dim=128**、**chunk_len=8**）；对每个 **goal 叶** 各跑一次 **`dfs_ssgs_mamba`**（**Mamba2Model** + `MambaNavState`，**必达**）与 **训练后 goal 子头贪心**（**可能未达**）；JSON 并列 **snapshots_taken / rollbacks / leaf_checks** 与 **LM** 的 **reached**、**child_choice_accuracy**、**num_internal_decisions**。与 **X-20260421-ssgs-mamba-dfs-demo** 不同：此处 **goal 可变**且树为 **文本形**；与 path-batch、§7.3.1 **仍非同一 harness**。登记 **X-20260425-ssgs-lm-nav-compare**；**CUDA 样例归档**：`results/metrics/ssgs_lm_nav_compare_default8_cuda.json`（**CPU** 可同命令加 `--cpu` 生成 `ssgs_lm_nav_compare_default8_cpu.json`）。
...
| S5 | 汇总表：**同等树深、同等试错次数** 下 SSM vs TF-R1 vs TF-KV（或两列 TF） | **部分**：§7.3.1 表 + JSON；**导航环**已接 **`dfs_ssgs_mamba`**；**真 LM**：**X-20260422** + **启发式 reach_rate**（**X-20260423**）+ **目标叶条件可学习子头**（**X-20260424**）+ **SSGS 与 LM 轨迹并列**（**X-20260425**）；**全拟合 / 统一 harness** 仍可扩展 |

**依赖关系**：S2/S3 依赖清晰的 **token 边界** 与 **路径枚举**（可与 `dfs_ssgs` 轨迹对齐）；S1 完成前，勿把玩具 `dim` 向量与「层状态字节数」混称为论文主表。

---

## 8. 树状 RAG / 多步修正：文献对齐与本文边界

**用途**：把「层次索引、多跳、自我修正」等 **Related Work 动机** 收束到 **可写进论文的表述**，并与本仓 **path-batch / SSGS / M1** **分列**；**不**新开独立文档。

### 8.1 树状 vs 平面检索（如 RAPTOR 系）

- **共识**：层次化组织（高层摘要 + 叶块证据）在长文档、多跳、需要主题级脉络的任务上 **常被** 用作补 **平面 top‑k chunk** 的信息结构；RAPTOR 等在 **特定数据集与强生成器设定** 上报告大幅提升——**引用须带原题设置**，**不可**泛化为「树状永远优于平面」。  
- **本文位置**：本仓提供 **Wikitext 浅树 + 同 harness path-batch** 与 **SSGS DFS 迹**；**若**要对标 RAPTOR 式 **准确率曲线**，需 **另设任务与 `kind`**（见 **`PLAN_NOW_TO_DONE` §Ⅷ** 工程门闩）。

### 8.2 回溯 / 自我修正（Self‑RAG、Adaptive‑RAG、纠错与再检索系）

- **共识**：多步 RAG 中「不满意则再检索/再生成」与 **决策修正** 在文献中 **反复出现**；与 **树上 DFS 走错再退** 在 **概念上**同属 **广义的试错—修正**，但 **实现**多为 **生成/检索循环**，**不等同**于 **`dfs_ssgs_mamba` 的隐状态快照**。  
- **本文位置**：**SSGS** 给出 **可测的** `snapshots_taken` / `rollbacks` / `leaf_checks`；**M1** 给出 **同树 DFS** 上 **SSGS vs 玩具 TF‑KV** 的 **路径级代价**——**与 Self‑RAG 类工作脚注分列**，**不作同构宣称**。

### 8.3 Mamba 固定状态 + 快照

- **叙事机会**：相对 **KV 随长增长**，**固定维隐状态 + clone/restore** 为 **高频回溯** 提供 **不同的可观测代价结构**；本仓已在 **玩具 trunk / path‑reader 协议** 上给出 **实测**（实现敏感、§7、M1）。  
- **外推**：**全规模 LLM + 与 HF 解码栈 iso‑协议** 的数字由 **`ENG-*` 工程线**（**`ENGINEERING_NORTH_STAR_PLAN.md`**）补齐；见 **`RESEARCH_STATUS` §6** 分档。

### 8.4 写作提示（一条）

动机段可 **正面** 引用 **§8.1–8.2** 建立 **问题重要性**；**实证句** 仍锚在 **登记与 JSON**（**`EXPERIMENT_REGISTRY`**、**七轴**），**RW 与 Results 勿混轴**。
