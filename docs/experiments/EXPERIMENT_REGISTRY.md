...
| X-20260423-tree-lm-nav-greedy | 2026-04-23 | 5060 **CPU**（`mamba2`） | `162a576` | A | **启发式树上导航任务**：每内部节点对各子 **path_to_document(walk+[ch])** 算 **CE**，**argmin** 下降；与 **金叶路径**（`iter_root_leaf_paths` 顺序）逐步对比 **`child_choice_accuracy`** / **`reached_target_leaf`**；**`--eval-all-leaves`** → **`reach_rate`** | `python scripts/research/demo_tree_lm_nav_greedy.py --cpu --eval-all-leaves --out-json results/metrics/tree_lm_nav_greedy_default8_cpu.json`；单叶：`--target-leaf-index k`（无 `--eval-all-leaves`） | 默认 **tiny-gpt2**、内置 8 句叶、**max_length=256**：**reach_rate=0.125**（**1/8** 叶到达）；**mean_child_choice_accuracy≈0.292**；**非**训练策略；金路径对齐用 **对象 `is`**（见 **RESEARCH_NOTES** §7.4）；JSON：`tree_lm_nav_greedy_default8_cpu.json` |
