...
| X-20260421-tf-kv-path-segments-cuda | 2026-04-21 | AutoDL / RTX 3090 **CUDA** | `6fa7873` | X | §7.5 S3 / §7.2 **TF-KV**：与 S1 **同路径**，Pre-LN 因果 trunk + **MHA KV cache**（非 `TransformerPathReader` 本体） | `benchmark_tf_kv_path_segments.py --device cuda --depth 4 --out-json results/metrics/tf_kv_path_segments_depth4_cuda_20260421.json` | `kv_cache_nbytes` **16 KiB→80 KiB**（seq 8→40，线性增长）；`increment_last_chunk_mean_ms` 约 **2→10**；`peak_alloc_mib` ≈**10.84–10.94** |
