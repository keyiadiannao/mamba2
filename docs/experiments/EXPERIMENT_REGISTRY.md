...
| X-20260421-mamba2-cache-segments-cpu | 2026-04-21 | 5060 / `mamba2` **CPU** | `54ce806+` | X | §7.5 S1：玩具树单路径，按节点 chunk **分段 forward + clone cache** | `benchmark_mamba2_cache_snapshot_segments.py --device cpu --depth 4 --out-json results/metrics/mamba2_cache_snap_segments_depth4_cpu_20260421.json` | 5 节点路径：每段后 **cache 约 41 KiB**（`batch=1`）；**3090 fused** 请另跑 `--device cuda` 并登记 |
...
| P1 | S1-cache-segment-benchmark | 云端跑 **`benchmark_mamba2_cache_snapshot_segments.py --device cuda --out-json ...`** 后登记；已完成：大叶数 **A-20260408-research-large-leaves-3090**、探针 **X-20260408-probe-mamba2-3090-fused**、分段脚本已入库 |
