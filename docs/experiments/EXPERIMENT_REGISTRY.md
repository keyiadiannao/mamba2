...
| X-20260421-ssgs-mamba-dfs-demo | 2026-04-21 | 5060 **CPU** + AutoDL / RTX 3090 **CUDA**（`mamba2`） | `d451c5b` | X | SSGS **`dfs_ssgs_mamba`** + 玩具 **`Mamba2Model`**：**token** 步进 + **`DynamicCache`** clone / zero_ / copy_；CUDA 侧 **`patch_mamba2_model_use_torch_forward_only`**（避免 fused **`causal_conv1d`** 在 **batch=1** 步进上的 stride 约束） | `python scripts/research/demo_ssgs_mamba_dfs.py --device cpu`；`python scripts/research/demo_ssgs_mamba_dfs.py --device cuda`（默认 depth=2、fanout=2、chunk_len=2、dim=64、layers=2，目标最右叶） | **`ok True`**；**snapshots_taken 3**、**rollbacks 4**、**leaf_checks 4**；**events** 前缀 **CPU/CUDA 一致**；stdout 指标（无固定 JSON）；**非** path-batch 扫参 harness |
