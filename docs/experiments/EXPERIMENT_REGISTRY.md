...
| X-20260408-probe-mamba2-3090-fused | 2026-04-08 | AutoDL / RTX 3090 | `88f33c8+` | X | §7.5 S0→S1：`Mamba2Model` 前向可观测字段 | `probe_mamba2_outputs.py --device cuda`；`--use-cache`；**`--dump-cache`** 打印每层 `conv_states` / `recurrent_states` 形状 | 无 `use_cache`：仅 `last_hidden_state`；`use_cache`：`cache_params=DynamicCache`，2×`LinearAttentionLayer`；**大叶数**见 **A-20260408-research-large-leaves-3090**；详细张量名与形状摘要见 **RESEARCH_NOTES §7.1** |
...
| P1 | S1-snapshot-at-tree-boundary | **下一步**：树节点边界上对 HF 层状态 **`conv_states`/`recurrent_states` `clone`** + **bytes**；已完成：大叶数 **A-20260408-research-large-leaves-3090**、探针 **X-20260408-probe-mamba2-3090-fused** |
