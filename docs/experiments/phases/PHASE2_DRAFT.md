...
- **辅线（SSGS × Mamba，与 path-batch 分列）**：**同 Wikitext 建树** 上 **`dfs_ssgs_mamba`**，归档 **`results/metrics_result/ssgs_mamba_wikitext_*.json`**、**`ssgs_mamba_wikitext_grid.csv`**；登记 **X-20260407-ssgs-mamba-wikitext-tree**；**`PHASE1_MANUSCRIPT.md` §4**、**`FIGURE_CAPTIONS_STAGE1.md`** **五条轴**。
