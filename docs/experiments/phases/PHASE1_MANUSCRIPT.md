...
> **正式体例「基础阶段」浓缩版**（**IMRaD**，便于汇报/扩写长文）：**`FOUNDATION_STAGE_FORMAL_SUMMARY.md`**（与本文 **互补**：本文 **全细节**，该文件 **论文骨架 + 分列结论**）。**主线叙事链 + 数据登记索引**：**`docs/overview/planning/NARRATIVE_MAINLINE_TREE_READER_SSGS.md`**。  
...
## 摘要（投稿版，与 **`NARRATIVE_MAINLINE_TREE_READER_SSGS.md` §0.1** 同步）
...
> **细节与格点数字**（阶段 2 叶扫、dim256、A2-S3 等）仍见 **§5、§8**；**勿在摘要堆砌 MiB 格点**，数值以 **`EXPERIMENT_REGISTRY`** 与 **`metrics_result`** 为准。
...
| **SSGS Wikitext** | **齐** | **`ssgs_mamba_wikitext_grid.csv`**（通配合并 **16 行** 量级）+ **`ssgs_mamba_wikitext_*.json`**（含 **n128 `20260410T1238Z`**）；辅 **`benchmark_wikitext_ssgs_bundle_20260410T0803Z_n8_c8.json`**（path-batch **同树** smoke，**与 SSGS 分列**） |
| **M1（SSGS vs TF-KV DFS）** | **齐** | **`ssgs_vs_kv_wikitext_nav_grid.csv`**（通配 **`ssgs_vs_kv_tree_nav_wikitext_*.json`**，**N 数据行 + 表头**，**N** = 聚合 **`ssgs_vs_kv_tree_nav_wikitext_*.json`** 个数）+ 多 **STAMP** JSON；登记 **X-ssgs-vs-kv-tree-nav-m1**；**`FIGURE_CAPTIONS_STAGE1.md`** **M1 轴** |
...
### 5.2 投稿包 §A3 / §A3b（可迁入 LaTeX/Word 脚注或附录）

> 与 **`docs/overview/execution/SUBMISSION_PACK.md` §A3 / §A3b** 同步；修订以该文件为准。满足 **`RESEARCH_PHASES_0_TO_DONE.md` 阶段 5**「§A3 入稿」勾选。

#### 中文（§A3）

**一段边界（Related Work 或 Method 脚注）**
...
**5060 与 3090**  
**5060** 上 **HF naive** 的 Wikitext 动机数字 **仅**作 **本地动机/趋势**；**3090 fused** 主文格点 **须分列脚注**，**禁止**无标注混点（本节 **§5.1**）。
...
**Seven one-sentence axis guards** — 全文见 **`SUBMISSION_PACK.md` §A3b**（含 **5060 vs 3090** 一句）。
...
## 7 英文摘要（与 **`NARRATIVE_MAINLINE_TREE_READER_SSGS.md` §0.2** 同步）
...
**本版成文**：**§8.4** 与 **§5** 已收口 **A2-S2 / dim256 / 叶数 8–256 / §7 depth 5–6**；**A2-S3** **3090** **`init_seed`×5**（**n16/n32**、**H=6**）JSON 含 **`1438Z`** 与 **复跑 `0820Z`/`0850Z`**，**TSV 摘要** 已入 **`results/metrics_result/`**，登记 **A-stage2-wikitext-path-pair-initseed5-3090-v1**。**dim256** 增补 **`0847Z` minimal**；**SSGS** **`ssgs_mamba_wikitext_grid.csv`** **16 行**（**`EXPERIMENT_REGISTRY` X-20260407**）。
...
1. **成文整合（阶段 5）**：以 **`SUBMISSION_PACK.md` §A2–A2.1** 为 **文件名真相源**，将 **`PHASE1_MANUSCRIPT` §5.2 / §8–§9**、**`EXPERIMENT_REGISTRY.md`**、**`results/metrics_result/`** 与 **LaTeX/Word** **逐字对齐**（含 **七轴图注**（**M1** + **L3 轨迹**）、**5060 vs 3090** 分列、**`json_path` basename 脚注**）；勾选 **`RESEARCH_PHASES_0_TO_DONE.md` 阶段 5**（**§A3** 边界段已入 **§5.2**）。
...
5. **SSGS（辅线，非阻塞）**：**Wikitext 同树** 已归档 **`ssgs_mamba_wikitext_grid.csv`**（通配合并 **16 行**，含 **n128** 与 **n8–64** 多 **STAMP**，登记 **X-20260407-ssgs-mamba-wikitext-tree**）。可选：**`git pull` 后** 重跑 **一格** 刷新 **`git_sha`**；玩具树 **X-20260421**、LM 并列 **X-20260425** 仍足 **附录** 基线。  
...
| 2026-04-10 | **§4 / §5 / §8.2 / §8.3 / §10**：**3090** **A2-S3** 复跑 **0820Z/0850Z** + **TSV**；**dim256** **`0847Z` minimal**；**SSGS grid** 后扩至 **16 行**；**§8.2** 补 **A2-S3 研究含义**（与墙钟 **分列** 的 **表征可读性 proxy**） |
...
| 2026-04-11 | **§5.1**：**`DATA_ARCHIVE_202604_SERVER.md`** 指针；**SSGS grid 16 行**、**B-S2+ 3090** 行；**聚合 `json_path`** 脚注（**相对仓 POSIX**） |
...
| 2026-04-11 | **§5.2**：**`SUBMISSION_PACK` §A3 / §A3b** 迁入（中英边界段 + 七轴；英轴全文仍见 **`SUBMISSION_PACK`**）；**`PLAN_NOW_TO_DONE`** 本周项 **Ⅰ-2** / **Ⅱ** / **Ⅲ-1** 勾选 |
| 2026-04-11 | **摘要**：**`NARRATIVE_MAINLINE_TREE_READER_SSGS.md` §0** 可扩写稿；篇首 blockquote 指针 |
| 2026-04-11 | **摘要 / §7 英文**：与 **`NARRATIVE` §0.1 / §0.2** 全文同步；**关键词** 入稿 |
| 2026-04-11 | **§4 SSGS**：**`…T0303Z` / `…T0334Z.json`** 入 **`metrics_result`**；**grid** **16** 行（**`aggregate_*` stdout**）；**主线完成度** 见 **`NARRATIVE_MAINLINE_TREE_READER_SSGS.md` §8** |
