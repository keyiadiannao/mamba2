...
- [x] **S3 TF-KV 脚本**：`benchmark_tf_kv_path_segments.py`（线性路径 KV + 增量 chunk；可选错枝截断）；**待**3090 JSON 与 `EXPERIMENT_REGISTRY`
...
| P1 | **测量协议成文** | §7.1 + S1/S2 已登记；**S3** `benchmark_tf_kv_path_segments.py` 跑 CUDA JSON 后登记；再后 SSGS restore / §7.3 三方表汇总 |
