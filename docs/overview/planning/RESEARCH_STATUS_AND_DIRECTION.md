...
### 6.4 **M1 玩具全链（①②③）之后**：「能用」、公平性与三风险实测

> **读者场景**：已在 **M1 harness** 上跑通 **Wikitext 建树 → DFS 三臂 →（可选）L3 下游 CE**，希望推进到 **更像真实系统** 的验证，并认为 **当前对比不够公平、需要上大模型**。本节把 **合理诉求** 与 **常见误解** 分开写清，避免 **跳过 harness 设计直接上 7B**。

**你的直觉里「对」的部分**

- **「链条要能用」**：玩具链 **只证明可实现性与部分表示一致性**（**L1–L3 的子集**），**不**承诺 **长上下文、真实检索噪声、学习式策略** 下的行为。要往 **可用** 走，必然需要 **更大规模、更贴近任务的指标** 与 **另开登记 / 新 `kind`**（见 **§6.3 档 2–3**）。  
- **「比较要更公平」**：**公平不是「把 Mamba 与 TF-KV 标成同一 FLOPs」** —— 在 **M1** 里两臂 **本就不是同一 trunk 与步长**，主文应 **并列代价结构** 并 **脚注分列**（**§6.1**）。**更公平的下一步** 是 **先固定「比什么」**：例如 **同一 token 预算**、**同一峰值显存上限**、**同一任务成功率定义**，再 **在同一 harness 槽位内** 换 **更大 path encoder** 或 **冻结 LM**，而不是混表 **小 encoder path-batch** 与 **全模型 KV**。  
- **「三风险要测」**：**§3.5** 三条（**有损状态 / 谁触发回退 / H2D**）**不应只停留在局限段**；若论文主叙事升级到 **「回溯可依赖」**，需要 **对应测量**（下表）。

**需要小心的误解**

- **误解**：「玩具不公平 → 上大模型就自动公平」。**实际**：模型变大后 **变量更多**（冻结？LoRA？KV 全长？），**若不先写清预算与任务**，更容易 **不可比**。应 **沿用 §6.3 分档**：先在 **path-reader 内** 加 **dim/层/序列**，再 **冻结中等 LM**，最后才 **端到端**。  
- **误解**：「M1 的 wall_s 可以直接论证 KV 优于 SSM」。**实际**：**跨臂墙钟不对等** 已写入 **`SSGS_MAINLINE_M1.md`** / **`SUBMISSION_PACK`**；升级叙事时应 **换「同预算」指标**（例如 **达到同一任务精度所需的回溯次数 / 峰值 / 总 token**）。

**三风险 → 建议的「可发表」测法（均需新行登记；禁止与主图无脚注合并）**

| 风险 | 测什么（思路） | 与当前仓关系 |
|------|----------------|--------------|
| **有损压缩** | **restore / 快照** 后 **任务指标** 相对 **金路径** 的 **落差**；或 **序列加长** 后 **L3 类指标** 是否劣化；**状态量化/截断** ablation | **M1 L3 CE** 已覆盖 **「表示对齐」** 的一小块；**任务级** 需 **A2-S3 类** 或 **树 LM 导航** 升级 **`kind`** |
| **回退触发** | **规则 DFS（SSGS）** vs **贪心/学习子头** 在 **同一棵树、同一任务** 上的 **成功率 / 代价** | **X-20260423/25** 为 **tiny** 基线；**加强子头** 见 **`PROJECT_MASTER_PLAN` §1.0** 副线 **B**，**非** 主文默认 |
| **H2D / 搬运** | 快照 **CPU↔GPU** 的 **字节数 + 墙钟**；与 **§7 S4 fromcpu restore** 同 spirit | 可在 **SSGS / Mamba cache** 上 **显式打点**（新脚本或扩展 JSON **`kind`**） |

**与阶段 5 成文的关系**：**不矛盾**。当前 **仍可** 以 **L1–L3 已有轴** 投稿；**§6.4** 所述为 **审稿后或第二篇** 的 **北星增量**，与 **`PLAN_NOW_TO_DONE.md` §Ⅵ** 同步。
...
| 2026-04-11 | **§6.4**：**M1 全链之后** — 「可用」、**公平性再定义**、**三风险实测** 路线；与 **`PLAN_NOW_TO_DONE` §Ⅵ** 互链 |
