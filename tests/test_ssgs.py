...
class TestSSGSDryRun(unittest.TestCase):
    ...
    def test_tensor_state_finds_rightmost_leaf(self) -> None:
        dim = 16
        root = build_balanced_tree(2, 2, chunk_len=3, dim=dim, device=torch.device("cpu"))
        leaves = [path[-1] for path in iter_root_leaf_paths(root)]
        target = leaves[-1]
        ...
        st = TensorNavState.zeros(dim, torch.device("cpu"))
        tr = SSGSTrace()
        ok = dfs_ssgs_tensor(root, st, leaf_goal=leaf_goal, trace=tr)
        self.assertTrue(ok)
        self.assertEqual(tr.snapshots_taken, 3)
        self.assertGreater(tr.rollbacks, 0)
