...
| **Linux** | **[server/run_m1_ssgs_vs_kv_wikitext_cuda.sh](server/run_m1_ssgs_vs_kv_wikitext_cuda.sh)** | **Phase M1**：**`benchmark_ssgs_vs_kv_tree_nav_wikitext.py`** **CUDA**；默认 **n8/n16/n32** **三臂**（可 **`M1_LEAVES`** / **`M1_NO_TRUNCATE=1`**）；可选 **`M1_WITH_L3`** / **`M1_WITH_L3_DOWNSTREAM_CE`**；末步 **`aggregate_ssgs_vs_kv_wikitext_json.py`** → **`ssgs_vs_kv_wikitext_nav_grid.csv`**（**`SKIP_M1_AGGREGATE=1`** 可关） |
